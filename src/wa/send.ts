...
export async function attemptSend(
  ...
  const now = deps.now();
  const accountVerdict = canSendNow(
    deps.store.loadAccountHealth(now),
    deps.config,
    now,
  );
  ...
  // Se re-lee después de persistir para que la evaluación vea exactamente el
  // estado que verá el próximo intento.
